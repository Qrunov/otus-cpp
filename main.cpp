...
private:
	...
	bool operator==(const allocator_t &a) const { return m_pool == a.m_pool; }
	bool operator!=(const allocator_t &a) const { return !(*this == a); }
};
	...
	void reserve(size_type new_capacity)
	{
		if (new_capacity > m_count)
		{
			...
			if (m_count)
			{
				try
				{
					uninitialized_copy(m_array, m_array + m_size, new_array);
				}
				catch (...)
				{
					allocator_traits<Alloc>::deallocate(m_a, new_array, new_capacity);
					throw;
				}
				for (size_type i = 0; i < m_size; ++i)
					allocator_traits<Alloc>::destroy(m_a, m_array + i);
...
int main()
{
	...
	auto print_map = [](auto &map)
	{
		...
		for (int i = 2; i < high; i++)
			map[i] = map[i - 1] * i;
		for_each(map.cbegin(), map.cend(), [](auto &a)
	...
	auto print_vector = [](auto &vector)
	{
		...
		for_each(vector.cbegin(), vector.cend(), [](auto &a)
				 { cout << a << " "; });
		cout << endl;
	};
	print_vector(s);
